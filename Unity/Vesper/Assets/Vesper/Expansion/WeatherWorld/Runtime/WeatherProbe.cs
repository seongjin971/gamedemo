...
public sealed class WeatherProbe:MonoBehaviour {
 ...
 IEnumerator Place(float d){motor.Stop();motor.transform.SetPositionAndRotation(WorldLayout.Point(d),Quaternion.Euler(0,180,0));control.SnapToPlayer();yield return new WaitForSecondsRealtime(4);}
 ...
 IEnumerator Pictures(){env.FlashOverride=0;foreach(float d in new[]{323f,282,258,25,60,84,104,122,150,198}){yield return Place(d);yield return Snap("weather-"+d);}env.FlashOverride=1;env.StrongOverride=false;yield return new WaitForSecondsRealtime(.1f);yield return Snap("abbey-fullscreen-flash");env.StrongOverride=true;yield return new WaitForSecondsRealtime(.1f);yield return Snap("abbey-strong-flash");env.FlashOverride=0;yield return new WaitForSecondsRealtime(1);yield return Snap("abbey-after-flash");yield return Place(150);env.FlashOverride=1;env.StrongOverride=false;yield return new WaitForSecondsRealtime(.1f);yield return Snap("rainwood-fullscreen-flash");env.FlashOverride=0;}
 ...
 IEnumerator Effects(){
  env.FlashOverride=0;yield return Place(323);
  Check(env.snow.particleCount>4096,"Fine snow exceeds old 4096 particle capacity: "+env.snow.particleCount);
  Check(env.nearSnow.particleCount>1200,"Near snow exceeds old 1200 particle capacity: "+env.nearSnow.particleCount);
  Check(!env.snow.collision.enabled&&!env.nearSnow.collision.enabled,"Snow uses no per-flake physics");
  yield return Place(198);float sunBase=env.sun.intensity;
  using(var trace=new StreamWriter(output+"/pulse-timing.csv")){
   trace.WriteLine("strong,time,flash,sunIntensity");
   foreach(bool stronger in new[]{false,true}){
    env.FlashOverride=-1;env.PreviewFlash(stronger);float start=Time.time,peak=0,secondPeak=0,tail=1;bool directionless=true;
    while(Time.time-start<1.25f){yield return null;float elapsed=Time.time-start;peak=Mathf.Max(peak,env.Flash);if(elapsed>.26f&&elapsed<.45f)secondPeak=Mathf.Max(secondPeak,env.Flash);if(elapsed>1)tail=Mathf.Min(tail,env.Flash);directionless&=Mathf.Abs(env.sun.intensity-sunBase)<.00001f;trace.WriteLine(FormattableString.Invariant($"{stronger},{elapsed:F5},{env.Flash:F5},{env.sun.intensity:F5}"));}
    Check(peak>.93f&&secondPeak>.65f&&tail<.001f,"Two short global pulses and darkness return, strong="+stronger+", peaks="+peak+"/"+secondPeak+", tail="+tail);
    Check(directionless,"Directional sunlight unchanged throughout flash, strong="+stronger);env.FlashOverride=0;yield return new WaitForSecondsRealtime(.5f);
   }
  }
 }
 IEnumerator Start(){yield return new WaitForSecondsRealtime(2);motor=FindAnyObjectByType<WorldMotor>();if(!motor){Application.Quit(2);yield break;}control=FindAnyObjectByType<WorldCamera>();run=motor.GetComponent<WorldRunInput>();env=FindAnyObjectByType<WorldEnvironment>();camera=control.GetComponent<Camera>();control.nativeInput=false;run.keyboardEnabled=false;if(mode=="-weatherCapture")yield return Pictures();else if(mode=="-weatherTraverse")yield return Traverse();else if(mode=="-weatherPerformance"){yield return Performance();yield return Effects();}else yield return Motion();Save();yield return new WaitForSecondsRealtime(.5f);Application.Quit(failures==0&&errors.Count==0?0:1);}
}
}
