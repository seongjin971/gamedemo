...
half4 frag(V i):SV_Target{
 float worldD=-i.w.z; float d=worldD<=48?worldD:worldD<138?48+(worldD-48)*42/90:worldD-48;float2 ground=i.w.xz,uv=ground*_TextureScale;
 ...
 half wet=smoothstep(98,156,worldD)*(1-smoothstep(224,264,d));
 ...
 color+=albedo*WeatherSurfaceLight(i.w)+WeatherWetFlash(n,wet*(.25+puddle*.75)*(1-roadSnow));
 return half4(WeatherFog(color,i.fog,i.w),alpha);
}
ENDHLSL
}
}
