...
namespace Vesper.Expansion.Editor {
    ...
    public static class RunAdventurerBuild {
        public const string Scene = "Assets/Vesper/Scenes/Expansion/VesperAdventurerRun.unity";
        const string Art = "Assets/Vesper/Expansion/P1Run/Art/Adventurer.fbx";
        [Serializable] class Calibration { public float walkSpeed, runSpeed, runPhaseOffset; }
        [Serializable] class ImportReport {
            public string scene, model, walk; public int skins, bones, triangles, missingScripts;
            public float authoredSpeed, authoredRunSpeed, runPhaseOffset; public string[] clips;
        }
        public static void Prepare() {
            try {
                ...
                foreach (var clip in specs) {
                    ...
                    clip.name = take.Contains("Run") ? "Run" : take.Contains("WalkAlternative") ? "WalkAlternative" : take.Contains("Idle") ? "Idle" : take.Contains("Stop") ? "Stop" : "Walk";
                ...
                var scene = EditorSceneManager.OpenScene(MixamoAdventurerBuild.Scene);
                ...
                visual.name = "Walking and running traveler"; visual.transform.SetParent(motor.transform, false); visual.transform.localScale = scale;
                ...
                var input = motor.gameObject.AddComponent<AdventurerRunInput>(); input.motor = motor;
                var motion = visual.AddComponent<AdventurerRunAnimation>(); motion.motor = motor; motion.input = input; motion.animator = animator;
                string selected = "Walk";
                motion.idleClip = clips.First(c => c.name == "Idle"); motion.walkClip = clips.First(c => c.name == selected); motion.runClip = clips.First(c => c.name == "Run");
                var calibration = JsonUtility.FromJson<Calibration>(File.ReadAllText("../../Migration/Source/Expansion/P1-Run/prepared-01/calibration.json"));
                ...
                if (calibration.runSpeed < .5f) throw new Exception("Invalid running stride calibration");
                motion.authoredRunSpeed = calibration.runSpeed * scale.x; motion.runPhaseOffset = calibration.runPhaseOffset;
                visual.AddComponent<AdventurerAmbient>();
                motion.idleClip.SampleAnimation(visual, 0);
                AssetDatabase.SaveAssets(); EditorSceneManager.SaveScene(scene);
                string output = Path.GetFullPath(AdventurerBuild.Arg("-p1Output", "../../Migration/Evidence/Expansion/P1-Run/candidate-01"));
                ...
                File.WriteAllText(Path.Combine(output, "character-import.json"), JsonUtility.ToJson(new ImportReport {
                    ...
                    triangles = skins.Sum(s => s.sharedMesh.triangles.Length / 3), authoredSpeed = motion.authoredWalkSpeed, authoredRunSpeed = motion.authoredRunSpeed, runPhaseOffset = motion.runPhaseOffset,
                ...
                Debug.Log("RUN_PREPARE_PASS"); EditorApplication.Exit(0);
        ...
        public static void Build() {
            string output = Path.GetFullPath("Builds/VesperRun/VesperRun.exe"); Directory.CreateDirectory(Path.GetDirectoryName(output));
            ...
            Debug.Log("RUN_BUILD_" + report.summary.result); EditorApplication.Exit(report.summary.result == BuildResult.Succeeded ? 0 : 1);
        }
    }
}
