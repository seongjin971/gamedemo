...
half4 frag(V i):SV_Target{
 ...
 half absorption=1-exp(-thickness*.30);half3 body=lerp(behind,lerp(_ShallowColor.rgb,_DeepColor.rgb,saturate(thickness*.28)),absorption);
 Light light=GetMainLight(TransformWorldToShadowCoord(i.w));half3 h=normalize(view+light.direction);half spec=pow(saturate(dot(n,h)),650)*.40;
