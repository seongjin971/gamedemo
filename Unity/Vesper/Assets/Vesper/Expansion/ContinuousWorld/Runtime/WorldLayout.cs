...
public static class WorldLayout {
 ...
 public static float Center(float d){if(d>=166&&d<=198)return 18;if(d>145&&d<166)return Mathf.Lerp(0,18,Mathf.SmoothStep(0,1,Mathf.InverseLerp(145,166,d)));if(d>198&&d<210)return Mathf.Lerp(18,10,Mathf.SmoothStep(0,1,Mathf.InverseLerp(198,210,d)));for(int i=1;i<Bends.Length;i++)if(d<=Bends[i].y)return Mathf.Lerp(Bends[i-1].x,Bends[i].x,Mathf.SmoothStep(0,1,Mathf.InverseLerp(Bends[i-1].y,Bends[i].y,d)));return Bends[Bends.Length-1].x;}
 ...
 public static float NearShore(float x)=>41.5f+ShoreShift(x)+1.5f*Mathf.SmoothStep(0,1,Mathf.InverseLerp(3.2f,6,Mathf.Abs(x-Center(48.5f))))-6*Mathf.SmoothStep(0,1,Mathf.InverseLerp(4.5f,8,Center(48.5f)-x));
 public static float FarShore(float x)=>55.5f+ShoreShift(x)+4.5f*Mathf.SmoothStep(0,1,Mathf.InverseLerp(3.2f,6,Mathf.Abs(x-Center(48.5f))));
 ...
 public static float Height(float x,float d){
  ...
  float overlookEdge=8.7f+1.2f*Mathf.Sin(d*.18f)+.6f*Mathf.Sin(d*.47f);
  float natural=level-BridgeRise(d)*Mathf.SmoothStep(0,1,Mathf.InverseLerp(3.4f,7,dist))+terrain*fringe+mass-24*Mathf.SmoothStep(0,1,Mathf.InverseLerp(385+1.8f*Mathf.Sin(x*.28f),405+2.2f*Mathf.Sin(x*.2f),d));
  natural-=24*Mathf.SmoothStep(0,1,Mathf.InverseLerp(355,372,d))*Mathf.SmoothStep(0,1,Mathf.InverseLerp(overlookEdge,overlookEdge+12,Center(d)-x));
  ...
  // The riverbed is already lowered by the early River branch. Keep both exact
  // bridge-end vertices at deck height so the adjoining terrain triangle has no step.
  return natural;
 }
