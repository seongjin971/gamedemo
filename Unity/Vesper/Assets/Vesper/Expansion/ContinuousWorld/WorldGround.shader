Shader "Vesper/WorldGround" {
Properties { _BaseMap("Surface atlas",2D)="white"{} _SnowMap("Snow surface",2D)="white"{} _PackedMap("Compacted snow",2D)="white"{} _StoneMap("Stone surface",2D)="white"{} _BaseColor("Tint",Color)=(1,1,1,1) _DetailScale("Detail scale",Float)=.27 _NormalStrength("Normal detail",Float)=.28 _Diagnostic("QA channels",Float)=0 }
...
half4 frag(V i):SV_Target{
 ...
 half3 soil=tile(uv,float2(0,.5));half3 stone=SAMPLE_TEXTURE2D(_StoneMap,sampler_StoneMap,uv*.67).rgb;half3 ice=SAMPLE_TEXTURE2D(_SnowMap,sampler_SnowMap,uv*.8).rgb;
 half3 grass=soil*half3(.49,.68,.38);half3 dry=lerp(grass,soil*half3(.82,.81,.74),path);dry=lerp(dry,stone*.75,rock);
 half3 packed=SAMPLE_TEXTURE2D(_PackedMap,sampler_PackedMap,uv).rgb;half3 packedSnow=lerp(ice,packed,path*.85)*.98*lerp(half3(1,1,1),half3(.55,.61,.69),path*.95);half3 albedo=lerp(dry*(1-wet*.3),packedSnow,snow)*_BaseColor.rgb;
