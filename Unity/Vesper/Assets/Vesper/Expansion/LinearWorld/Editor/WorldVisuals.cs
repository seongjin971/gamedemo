...
public static partial class WorldVisuals {
 ...
 static Material Foliage(bool birch){
  ...
  var m=Mat(key, birch?new Color(.48f,.53f,.39f):new Color(.66f,.72f,.68f));m.shader=Shader.Find("Vesper/WorldFoliage");m.SetTexture("_BaseMap",tex);m.SetFloat("_Cutoff",birch?.4f:.35f);m.SetFloat("_Cull",0);m.SetFloat("_Wind",.035f);m.EnableKeyword("_ALPHATEST_ON");m.renderQueue=2450;return m;
 }
 static Material Slot(string name){
  if(name.Contains("LinearGrass"))return Mat(name,name.Contains("Dry")?new Color(.43f,.39f,.22f):name.Contains("Light")?new Color(.38f,.43f,.20f):new Color(.28f,.37f,.15f),.07f);
 ...
 static GameObject Model(Transform parent,string file,Vector3 pos,Vector3 scale,float yaw,bool blocker=false,bool hf=false){
  ...
  if(file.StartsWith("VisualF/")||file.StartsWith("L02Details/")||file.StartsWith("L04Details/"))path=LinearBuild.Root+"/Art/EnvironmentKit/"+file+".fbx";var asset=AssetDatabase.LoadAssetAtPath<GameObject>(path);if(!asset){throw new Exception("Missing required model "+path);}
