Shader "Vesper/Linear/River" {
Properties { _ShallowColor("Shallows",Color)=(.13,.37,.35,1) _DeepColor("Deep water",Color)=(.035,.16,.19,1) _ReflectionStrength("Reflection",Range(0,1))=.8 _Debug("Diagnostic",Float)=0 }
...
half4 _ShallowColor,_DeepColor;half _ReflectionStrength,_Debug;
CBUFFER_END
...
half4 frag(V i):SV_Target{
 ...
 float2 ripple=waterGradient(flow*.83)*.12+waterGradient(flow*2.71+17.4)*.055+waterGradient(flow*24.17-4.8)*.038;
 ...
 ruv.y=1-ruv.y;
 #endif
 ruv+=n.xz*.022;half3 reflected=SAMPLE_TEXTURE2D(_VesperPlanarReflection,sampler_VesperPlanarReflection,ruv).rgb;reflected=reflected/(1+reflected);
 half3 behind=SampleSceneColor(uv+n.xz*.006*saturate(thickness));behind=behind/(1+behind);
 half absorption=.52+.4*(1-exp(-thickness*.65));half3 body=lerp(behind,lerp(_ShallowColor.rgb,_DeepColor.rgb,saturate(thickness*.30)),absorption);
 Light light=GetMainLight(TransformWorldToShadowCoord(i.w));half3 h=normalize(view+light.direction);half spec=pow(saturate(dot(n,h)),1800)*.028;
 body*=lerp(.64,1,light.shadowAttenuation);
 ...
 if(_Debug>.5&&_Debug<1.5)return half4(saturate(thickness*.3).xxx,1);
 if(_Debug>1.5&&_Debug<2.5)return half4(behind,1);
 if(_Debug>2.5&&_Debug<3.5)return half4(reflected,1);
 if(_Debug>3.5)return half4(body,1);
 return half4(MixFog(min(color,half3(.72,.83,.86)),i.fog),1);
}
ENDHLSL
}
}
