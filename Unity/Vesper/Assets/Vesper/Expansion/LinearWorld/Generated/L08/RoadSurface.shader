...
Properties {
 ...
 _TextureScale("Meters to texture",Float)=.333333 _NormalStrength("Surface relief",Float)=8
}
...
half4 frag(V i):SV_Target{
 ...
 half stoneAmount=lerp(.28,.69,smoothstep(34,94,d))+abbey*.13;
 stoneAmount*=1-smoothstep(1.18,2.28,across)*.68;
 half stoneMix=smoothstep(.22,.82,stoneAmount+(variation-.5)*.60);
 ...
 half3 rainy=lerp(mud,rock*half3(.78,.81,.83),stoneMix*.66);
 half3 albedo=lerp(dry,rainy,wet);
 half iceProgress=smoothstep(198,272,d);
 half roadSnow=smoothstep(.12,.93,iceProgress+(variation-.5)*.40+smoothstep(.85,2.3,across)*.16);
 ...
 albedo=lerp(albedo,thaw,slush*.68);
 ...
 half puddle=smoothstep(.61,.80,patches(ground*.66)+(variation-.5)*.14)*wet*(1-stoneMix*.5)*(1-roadSnow);
 albedo*=lerp(1,.70,puddle);
 ...
 half3 reflected=SampleSH(half3(0,1,0))*half3(.86,.97,1.08);
 ...
 reflected=lerp(reflected,planar,availability*.65);
 color=lerp(color,reflected,puddle*(.28+.32*pow(1-saturate(dot(n,view)),3)));
