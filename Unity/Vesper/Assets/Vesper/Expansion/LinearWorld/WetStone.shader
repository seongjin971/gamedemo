...
half4 frag(V i):SV_Target{
 ...
 half snow=smoothstep(214,271,-i.w.z)*.93;albedo=lerp(albedo,SAMPLE_TEXTURE2D(_SnowMap,sampler_SnowMap,i.w.xz*.22).rgb*half3(.65,.71,.79),snow);wet*=1-snow;
 ...
 float3 toLamp=float3(4.05,4.2,-147.4)-i.w;half lamp=saturate(dot(n,normalize(toLamp)))*3/(1+dot(toLamp,toLamp));
