...
namespace Vesper.Expansion.P2 {
 public sealed class P2Performance:MonoBehaviour {
  ...
  string output,stage="warmup";bool manual,collectTiming;int unfocused;float start;Vector3 initial;P2Motor knight;P2Camera control;
  [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]static void Install(){var args=Environment.GetCommandLineArgs();int i=Array.IndexOf(args,"-p2Performance");bool manual=false;if(i<0){i=Array.IndexOf(args,"-p2ManualQA");manual=true;}if(i<0||i+1>=args.Length)return;var p=new GameObject("Temporary player verification").AddComponent<P2Performance>();p.output=args[i+1];p.manual=manual;p.collectTiming=args.Contains("-vesperTiming");Directory.CreateDirectory(p.output);}
  ...
  IEnumerator Start(){knight=FindAnyObjectByType<P2Motor>();control=Camera.main.GetComponent<P2Camera>();control.MatchP1Framing=true;initial=knight.transform.position;yield return new WaitForSecondsRealtime(5);Begin(manual?"manual-input":"static");if(manual){while(true){yield return new WaitForSecondsRealtime(1);Save();}}yield return new WaitForSecondsRealtime(8);yield return Capture("static");End();Begin("walk");knight.Walk(new Vector3(-3.08f,0,5.21f));yield return new WaitForSecondsRealtime(8);yield return Capture("walk");End();Begin("orbit");for(float t=0;t<8;t+=Time.unscaledDeltaTime){control.Orbit(Time.unscaledDeltaTime*.065f,Mathf.Sin(t)*Time.unscaledDeltaTime*.02f);yield return null;}yield return Capture("orbit");End();Begin("zoom");for(float t=0;t<8;t+=Time.unscaledDeltaTime){control.SetZoom(10.5f+Mathf.Sin(t*.8f)*2);yield return null;}yield return Capture("zoom");End();Save();Application.Quit();}
