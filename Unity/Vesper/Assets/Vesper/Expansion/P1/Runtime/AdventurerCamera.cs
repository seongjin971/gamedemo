...
namespace Vesper.Expansion {
    ...
    public sealed class AdventurerCamera : MonoBehaviour {
        public AdventurerMotor player;
            ...
            if (e.type == EventType.MouseDown) { press = lastMouse = pos; dragging = false; e.Use(); }
                ...
                if (!dragging && player) {
                    ...
                    if (new Plane(Vector3.up, Vector3.zero).Raycast(ray, out float distance)) {
                        if (!player.Walk(ray.GetPoint(distance))) blocked++;
                    }
                }
                e.Use();
            }
        ...
        void LateUpdate() {
            ...
            var follow = homeTarget + (player ? (player.transform.position - initialPlayer) * .24f : Vector3.zero);
