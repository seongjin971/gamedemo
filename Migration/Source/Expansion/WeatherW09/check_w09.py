...
if sys.argv[1]=='preserve':
    ...
    code=code.replace("E=ROOT/'Migration/Evidence/Expansion/WeatherWorld/W07'","E=ROOT/'Migration/Evidence/Expansion/WeatherWorld/W09'").replace("signature('W07')","signature('W09')").replace("'candidate':'W07'","'candidate':'W09'").replace("E/'before-source/Runtime'","ROOT/'Migration/Evidence/Expansion/WeatherWorld/W07/before-source/Runtime'")
    ...
    for kind in ['flash','strong']:
        old=regions('abbey-W06-'+kind);new=regions('abbey-W09-'+kind);normal=regions('abbey-W09-normal')
        r['comparisons'][kind]={'W06Quadrants':old,'W09Quadrants':new,'W09OverW06':[b/a for a,b in zip(old,new)],'W09OverNormal':[b/a for a,b in zip(normal,new)]}
    normal=regions('abbey-W09-normal');after=regions('abbey-after-flash');r['normalReturnRatios']=[b/a for a,b in zip(normal,after)]
    r['allQuadrantsNoticeablyBrighter']=all(v>1.25 for c in r['comparisons'].values() for v in c['W09OverW06'])
