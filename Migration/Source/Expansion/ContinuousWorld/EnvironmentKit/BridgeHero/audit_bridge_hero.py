...
SOURCE=Path(__file__).resolve().parent;ROOT=SOURCE.parents[5];OUT=ROOT/'Unity/Vesper/Assets/Vesper/Expansion/ContinuousWorld/Art/EnvironmentKit/BridgeHero'
...
# Source material side-view makes the open arch visible without any Unity scene edits.
bpy.ops.wm.open_mainfile(filepath=str(SOURCE/'StoneBridgeHero14m.blend'))
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=32;scene.render.resolution_x=1400;scene.render.resolution_y=900;scene.render.resolution_percentage=100
world=bpy.data.worlds.new('Bridge audit lighting');world.use_nodes=True;scene.world=world;world.node_tree.nodes['Background'].inputs['Color'].default_value=(.30,.35,.40,1);world.node_tree.nodes['Background'].inputs['Strength'].default_value=.75
ld=bpy.data.lights.new('Bridge side softbox','AREA');ld.energy=3000;ld.size=10;lo=bpy.data.objects.new('Bridge side softbox',ld);bpy.context.collection.objects.link(lo);lo.location=(10,-6,12);lo.rotation_euler=(Vector((0,0,0))-lo.location).to_track_quat('-Z','Y').to_euler()
cd=bpy.data.cameras.new('Bridge side camera');cam=bpy.data.objects.new('Bridge side camera',cd);bpy.context.collection.objects.link(cam);cam.location=(18,-6,4.5);cam.rotation_euler=(Vector((0,0,-.3))-cam.location).to_track_quat('-Z','Y').to_euler();cd.type='ORTHO';cd.ortho_scale=17;scene.camera=cam;scene.view_settings.view_transform='AgX';scene.render.image_settings.file_format='PNG';scene.render.filepath=str(SOURCE/'bridge-side-preview.png');bpy.ops.render.render(write_still=True)
