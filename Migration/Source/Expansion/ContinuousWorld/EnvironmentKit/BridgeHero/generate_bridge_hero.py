...
OUT=ROOT/'Unity/Vesper/Assets/Vesper/Expansion/ContinuousWorld/Art/EnvironmentKit/BridgeHero';OUT.mkdir(parents=True,exist_ok=True)
...
def mat(name,color):
    ...
    if ATLAS.exists():
        ...
        bump=nt.nodes.new('ShaderNodeBump');bump.inputs['Strength'].default_value=.15;bump.inputs['Distance'].default_value=.035;nt.links.new(tx.outputs['Color'],bump.inputs['Height']);nt.links.new(bump.outputs['Normal'],bs.inputs['Normal'])
...
def slab(x0,x1,y0,y1,seed):
    ...
    for poly in ob.data.polygons:
        for li in poly.loop_indices:
            ...
            uv.data[li].uv=(.025+.95*u,.025+.95*v2)
    return ob

# Mixed-size rectangular bonding on a fine planning grid. No continuous plank rows.
r=random.Random(2941);used=set();nx,ny=10,28;slab_rects=[]
for row in range(ny):
    for col in range(nx):
        ...
        x0=-2.6+col*.52+.010;x1=-2.6+(col+cw)*.52-.010;y0=-7+row*.5+.010;y1=-7+(row+ch)*.5-.010
...
def arch(t,outer=False):
    a=-math.pi/2+math.pi*t;return ((5.85 if outer else 5.6)*math.sin(a),-1.70+(1.55 if outer else 1.25)*math.cos(a))
...
core=extrude_yz('Continuous arched vault core',lower+[(5.6,-.15),(-5.6,-.15)],-3.14,3.14,2);objects.append(core)
for sign in (-1,1):
    ...
    cutpoints=[(-7,-3),(7,-3),(7,-1.70),(5.85,-1.70)]+[arch(i/40,True) for i in reversed(range(41))]+[(-7,-1.70)]
    cutter=extrude_yz('Spandrel cutter',cutpoints,-4,4,2)
    y=-7;row=0
    while row<5:
        z0=-1.70+row*.33+.006;z1=z0+.318;y=-7;col=0
        while y<6.99:
...
for y0,y1 in [(-7,-5.6),(5.6,7)]:objects.append(block('Bank abutment core',(-3.14,3.14,y0,y1,-1.72,-.17),seed=int(y0*99),slot=2,wear=0,bev=.008))

deck_max=max(v.co.z for ob in paving for v in ob.data.vertices)
bpy.ops.object.select_all(action='DESELECT')
for ob in objects:ob.select_set(True)
bpy.context.view_layer.objects.active=objects[0];bpy.ops.object.join();bridge=objects[0];bridge.name='CW_StoneBridgeHero14m'
...
under=[]
for y in (-4,-2,0,2,4):
    crown=-1.70+1.25*math.sqrt(1-(y/5.6)**2)
    for dz in (-.15,-.4):
...
manifest={'name':bridge.name,'length':14,'clear_width':5.2,'full_width':6.5,'deck_max_height':deck_max,'walking_top_contract':'0 nominal; all paving <= +0.008, no steps; parent smooth collider',
...
(SOURCE/'manifest.json').write_text(json.dumps(manifest,indent=2));(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2));bpy.ops.wm.save_as_mainfile(filepath=str(SOURCE/'StoneBridgeHero14m.blend'))
