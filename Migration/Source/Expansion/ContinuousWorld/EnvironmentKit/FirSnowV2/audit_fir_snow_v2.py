...
for mp in SOURCE.glob('*-manifest.json'):
    ...
    report['models'][m['name']]={'passed':passed,'triangles':tri,'zero_area_triangles':zero,'explicit_triangles':all(len(f)==3 for f in fs),'bounds_source_xyz':bounds,'reimport_bounds_error':err,'uv_bounds_per_material':slots,'fbx_root_rotations':rots,'wall_silhouette_height_samples':height_samples};report['passed'] &= passed
(SOURCE/'fir-snow-fbx-audit.json').write_text(json.dumps(report,indent=2));print('FIR_SNOW_V2_FBX_AUDIT_PASS='+str(report['passed']))
