...
for i in range(480):
    ...
    h=r.uniform(.62,1.18)*(1-.12*abs(y)/.75);a=r.uniform(0,math.tau);bend=Vector((math.cos(a),math.sin(a),0))*r.uniform(.025,.18);root=Vector((x,y,0));mid=root+Vector((0,0,h*.56))+bend*.25;tip=root+Vector((0,0,h))+bend
    tube([root,mid,tip],[.005,.0042,.0011],1 if i%5<2 else 0)
    for j in range(1+(i%2)):
        t=r.uniform(.28,.70);p=root.lerp(tip,t);angle=a+r.uniform(-2,2);d=Vector((math.cos(angle),math.sin(angle),.16));leaf(p,d,r.uniform(.17,.34),r.uniform(.009,.017),2 if i%3 else 1)
    if i%7==0:
        d=(tip-mid).normalized();tube([tip-d*.13,tip-d*.055,tip+d*.008],[.009,.014,.003],3)
...
full=build('CW_GoldenReedBand4m');low=build('CW_GoldenReedBand4m_LOD1',True);low.hide_render=True;bpy.ops.wm.save_as_mainfile(filepath=str(SOURCE/'GoldenReedBand.blend'))
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=48;scene.render.resolution_x=1500;scene.render.resolution_y=900;scene.render.resolution_percentage=100
world=bpy.data.worlds.new('Reed inspection');world.use_nodes=True;scene.world=world;world.node_tree.nodes['Background'].inputs['Color'].default_value=(.22,.30,.32,1);world.node_tree.nodes['Background'].inputs['Strength'].default_value=.65
ld=bpy.data.lights.new('Warm low waterside sun','AREA');ld.energy=1100;ld.size=5;lo=bpy.data.objects.new('Warm low waterside sun',ld);bpy.context.collection.objects.link(lo);lo.location=(-4,-3,6);lo.rotation_euler=(Vector((0,0,.5))-lo.location).to_track_quat('-Z','Y').to_euler()
cd=bpy.data.cameras.new('Reed camera');cam=bpy.data.objects.new('Reed camera',cd);bpy.context.collection.objects.link(cam);cam.location=(5,-8,4);cam.rotation_euler=(Vector((0,0,.6))-cam.location).to_track_quat('-Z','Y').to_euler();cd.type='ORTHO';cd.ortho_scale=5;scene.camera=cam;scene.view_settings.view_transform='AgX';scene.render.image_settings.file_format='PNG';scene.render.filepath=str(SOURCE/'reed-band-preview.png');bpy.ops.render.render(write_still=True);print('GOLDEN_REEDS_COMPLETE')
