"""Render the QA-passed V13 blend. Run only in the coordinated render slot."""
import bpy,json,math,hashlib
from pathlib import Path
from mathutils import Vector
OUT=Path(__file__).resolve().parent
qa=json.loads((OUT/'export-qa.json').read_text())
assert qa['passed'] and qa['mesh_sha256']==hashlib.sha256((OUT/'paving-v13-mesh.json').read_bytes()).hexdigest()
scene=bpy.context.scene;camera=scene.camera;PREVIEW=OUT/'previews'
oldob=bpy.data.objects['BEFORE V12 preserved'];objects=[ob for ob in scene.objects if ob.type=='MESH' and ob!=oldob]
neutral=bpy.data.materials['Neutral geometry only'];textured=bpy.data.materials['Actual source4K Color roughness NormalGL .4']
a=.46;e=.72
...
print('PAVING_V13_PREVIEWS_DONE',qa['mesh_sha256'])
