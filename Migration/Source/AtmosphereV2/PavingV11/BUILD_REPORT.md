...
SHA256: `8ba9fdd42a16f79f8c8f130161fed056d8043ffff1419ce4a23f024b8d5f757b`
...
| 독립 Pillow 높이 검증 | 6,053점, 최대 오차 5.01e-9m |
| UV 수식 최대 오차 | 5.44e-10 |
...
모든 채널은 `u=worldX/9.2`, `v=worldZ/4.6`를 공유한다. UV를 clamp하지 말고 repeat한다. 아트 배율은 원본2.3×1.15m의4.0배다. 행 offset, cutoff, 패치별 회전이 없다. Color는 sRGB, Roughness/Normal/Displacement는 비색상 데이터다. 재질 추가 UV배율은1, offset0으로 두어야 한다. 임의의 shader world좌표 배율을 덧씌우면 파손과 Color가 어긋난다.

높이는 `Y=-.0236+.040*(H-1)`로 계산했다. H는 원본16bit displacement의 반복 bilinear 샘플이다. **40mm는 아트 선택이며 실측 세로 범위가 아니다.** 장면별 최소/최대 정규화는 없다. 실제 격자 샘플은 원본의 최대값1을 정확히 밟지 않으므로 실제 Y범위는 **[-.059662172,-.024723317]**이다. -.0236은 상한이다. 독립 물 Y=-.004와 최소20.723mm 분리된다. 기존 V10 두께의 바닥값-.15676까지 채운 입체 석재가 아니라 얕은 열린 heightfield 표면이다. 측면·바닥은 생성하지 않았으며 경계 edge1628은 외곽/보존된 void의 정상 열린 경계다.
...
현재9.2×4.6m textured full/close와 pass2의4.6×2.3m 결과를 같은 카메라로 직접 비교했다. 직사각 판석의 넓은 면과 큰 파손이 분명히 커졌고, 표면의 작은 cobble 밀도는 줄었다. 상면에서 source pattern이 차지하는 면적은4배지만 stone/soil의 원본 비율 자체는 바꾸지 않았다. 중성 기하의 높이는40mm 그대로라 큰 판석의 면 기울기가 더 완만하다. 긴 띠 반복과 건조색은 여전히 보이며 이 교정의 native 크기/반사 결과는 root가 확인한다.
