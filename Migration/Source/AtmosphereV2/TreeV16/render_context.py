...
# Native camera right is left-handed: Unity right = up cross forward.
# Reflect the entire context X axis, then rebuild a proper RH Blender camera.
# A rotation-only look-at in the old (x,-z,y) bridge mirrors the native image.
# This changes only this temporary render scene, never delivery meshes/files.
...
def depth(o):
 ...
 return n
for o,mw in sorted(world_matrices,key=lambda item:depth(item[0])):o.matrix_world=mirror@mw
...
for name,loc,power,size in [('Cool broad',(-6,-1,15),2100,7),('Soft opposing',(11,5,10),1000,6)]:
 d=bpy.data.lights.new(name,'AREA');d.energy=power;d.size=size;o=bpy.data.objects.new(name,d);scene.collection.objects.link(o);o.location=(-loc[0],loc[1],loc[2]);o.rotation_euler=(Vector((-7,-3,4))-o.location).to_track_quat('-Z','Y').to_euler()
...
def camera(focus,angle,elev,ortho,native_frame=True):
 cd.sensor_fit='VERTICAL' if native_frame else 'HORIZONTAL'
 ...
 if native_frame:
  frame=cd.view_frame(scene=scene);height=max(v.y for v in frame)-min(v.y for v in frame);assert abs(height-ortho*2)<1e-4,('Native orthographic height mismatch',height,ortho*2)
...
for tag,ob in zip(['before','after'],trees):
 ...
 for name,focus,a,e,scale in views:
  ...
  camera(focus,a,e,scale,name!='roots')
  if name=='glare-orbit':
  ...
  if name!='roots':
   ...
   bpy.context.view_layer.update()
   native_cam=Vector((-cam.location.x,cam.location.z,-cam.location.y))
   native_right=Vector((-math.cos(a),0,-math.sin(a)))
   native_up=Vector((math.sin(a)*math.sin(e),math.cos(e),-math.cos(a)*math.sin(e)))
   for landmark in [(7.2,0,3.7),(0,2.9,.7),(-4,0,8)]:
    q=Vector(landmark);d=q-native_cam
    expected=(.5+d.dot(native_right)/(2*scale*1.5),.5+d.dot(native_up)/(2*scale))
    actual=world_to_camera_view(scene,cam,Vector((-q.x,-q.z,q.y)))
    error=max(abs(actual[i]-expected[i]) for i in range(2));assert error<1e-5,('Native projection mismatch',name,landmark,error)
    projection_checks.append(dict(view=name,state=tag,landmarkNativeWorld=list(q),expectedUnityViewport=list(expected),actualBlenderViewport=list(actual)[:2],maxError=error))
  scene.render.filepath=str(W/(name+'-'+tag+'.png'));bpy.ops.render.render(write_still=True)
(W/'preview-projection-qa.json').write_text(json.dumps(dict(coordinateBridge='Native world (-x,-z,y), reflected context; camera proper RH look-at',checks=projection_checks),indent=2))
