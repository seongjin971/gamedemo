...
def root_fields(q,k):
 ...
 t=smooth((r-.75*ends[k])/(.15*ends[k]));narrow=smooth((r-.55*ends[k])/(.40*ends[k]))
...
def roots(q,vertical=True):
 ...
 for k in range(4):
  ...
  aa=theta-(.65 if k==3 else .22)*diff*narrow-(.50*narrow if k==3 else 0)
...
for k in range(4):
 ...
 safe=min(supports[i] for i in valid)-.045;sink=max(.10,float(p[cap,2].max()-safe/scale));sinks.append(sink)
 rootplans.append(dict(root=k+1,role='parapet' if k==3 else 'floor',sourceCenterlineEndRadiusLocal=ends[k],descentStartRadiusLocal=.75*ends[k],fullDescentRadiusLocal=.90*ends[k],terminalVertexSamples=int(cap.sum()),supportSamples=len(valid),unsupportedTerminalSamples=len(supports)-len(valid),minimumActualSupportWorldY=min(supports[i] for i in valid),conservativeTargetTopWorldY=safe,downwardTranslationLocal=sink))
new=roots(p)
...
det=np.linalg.det(jac);assert det.min()>.025,('Invalid deformation Jacobian',float(det.min()))
