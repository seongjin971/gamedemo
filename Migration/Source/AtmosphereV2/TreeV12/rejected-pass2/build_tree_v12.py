...
local=spikeWeight*np.clip(prominence/.025,0,1)*.75
local*=np.minimum(1,.065/np.maximum(np.linalg.norm(res,axis=1),1e-8))
u-=res*local[:,None];p=u[inv]
...
p[:,2]+=.16*dominant*outer*rootWeight*(1-smooth((r-1.08)/.25))
terminal=smooth((r-1.08)/.30)*rootWeight
p[:,2]-=.25*terminal
# Preserve original global scale/origin and every upper-crown tip; no recenter.
# A source-face orientation barrier prevents local spur edits folding tiny
# triangles. Relax the displacement envelope near any offending triangle.
tris=np.array([t.vertices[:] for t in m.loop_triangles]);oc=np.cross(original[tris[:,1]]-original[tris[:,0]],original[tris[:,2]]-original[tris[:,0]])
delta=p-original;factor=np.ones(len(unique));barriercounts=[]
for iteration in range(80):
 trial=original+delta*factor[inv,None];nc=np.cross(trial[tris[:,1]]-trial[tris[:,0]],trial[tris[:,2]]-trial[tris[:,0]])
 cosine=(oc*nc).sum(axis=1)/np.maximum(np.linalg.norm(oc,axis=1)*np.linalg.norm(nc,axis=1),1e-20)
 bad=(cosine<.35)&(trial[tris,2].max(axis=1)>.02);barriercounts.append(int(bad.sum()))
 if not bad.any():break
 touched=np.unique(inv[tris[bad].ravel()]);factor[touched]*=.70
 # Carry the reduction to direct neighbors, avoiding a sharp safety boundary.
 neighborMinimum=factor.copy();np.minimum.at(neighborMinimum,aa,factor[bb]);factor=np.minimum(factor,.65*factor+.35*neighborMinimum)
p=original+delta*factor[inv,None]
assert barriercounts[-1]==0,barriercounts
assign(o,p)
