...
OUT=Path(__file__).resolve().parent;SRC=OUT.parent/'PavingV16/paving-v16-mesh.json';FILE=OUT/'paving-v17-mesh.json'
...
for part in m['parts']:
 if not part['name'].startswith('V17_'):continue
 ...
 if part['name'].startswith('V17_Hero'):
  s=next(s for s in m['selected_records'] if 'V17_'+s['name']==part['name']);poly=s['raw_polygon_world_xz'];area=abs(sum(a[0]*b[1]-a[1]*b[0] for a,b in zip(poly,poly[1:]+poly[:1])))*.5;q['expected_area']=area;q['area_error']=abs(area_top-area)
...
previous=json.loads((OUT.parent/'PavingV16/fracture-metadata.json').read_text())
for s in previous['selected_records']:outlines[s['name']]=s['raw_polygon_world_xz']
...
for part in m['parts']:
 if not part['name'].startswith('V17_joint_fill'):continue
...
# Independently emulate native float32 attributes before testing every triangle.
def f32(v):return struct.unpack('f',struct.pack('f',v))[0]
p32=[f32(v) for v in p];n32=[f32(v) for v in n]
qa['float32_nonpositive']=0;qa['float32_degenerate']=0
for t in range(0,len(ids),3):
 tri=ids[t:t+3];a,b,c=[p32[3*i:3*i+3] for i in tri];cr=cross(sub(b,a),sub(c,a))
 qa['float32_nonpositive']+=not all(dot(cr,n32[3*i:3*i+3])>0 for i in tri)
 qa['float32_degenerate']+=dot(cr,cr)<1e-24
outlines32={name:[[f32(x),f32(z)] for x,z in poly] for name,poly in outlines.items()}
qa['float32_outline_overlap_errors']=[]
for s in m['selected_records']:
 for name,poly in outlines32.items():
  if name!=s['name'] and overlap(outlines32[s['name']],poly):qa['float32_outline_overlap_errors'].append([s['name'],name])
qa['protected_v16_parts_exact']=all(any(q['name']==old['name'] and ids[q['index_start']:q['index_start']+q['index_count']]==before['indices'][old['index_start']:old['index_start']+old['index_count']] for q in m['parts']) for old in previous['parts'] if old['name'].startswith('V16_') or old['name'].startswith('HeroScanSlab_280_'))
qa['new_fill_count']=sum('fill' in q['name'] for q in qa['new_parts'])
qa['new_stone_count']=len(qa['new_parts'])
qa['passed'] &= not qa['float32_nonpositive'] and not qa['float32_degenerate'] and not qa['float32_outline_overlap_errors'] and qa['protected_v16_parts_exact'] and qa['new_fill_count']==0 and qa['new_stone_count']==18

qa['float32_new_body_closed']=True
for part in m['parts']:
 if not part['name'].startswith('V17_'):continue
 edge_count=Counter();directed_count=Counter()
 for t in range(part['index_start'],part['index_start']+part['index_count'],3):
  tri=[tuple(p32[3*i:3*i+3]) for i in ids[t:t+3]]
  for a,b in zip(tri,tri[1:]+tri[:1]):edge_count[tuple(sorted((a,b)))]+=1;directed_count[(a,b)]+=1
 qa['float32_new_body_closed'] &= all(v==2 for v in edge_count.values()) and all(v==directed_count[(b,a)] for (a,b),v in directed_count.items())
qa['preserved_fill_overlap_errors']=[]
for part in m['parts']:
 if not part['name'].startswith('V16_joint_fill'):continue
 poly=hull([(p32[3*i],p32[3*i+2]) for i in ids[part['index_start']:part['index_start']+part['index_count']]])
 for stone in m['selected_records']:
  if overlap(poly,outlines32[stone['name']]):qa['preserved_fill_overlap_errors'].append([part['name'],stone['name']])
qa['passed'] &= qa['float32_new_body_closed'] and not qa['preserved_fill_overlap_errors']

(OUT/'export-qa.json').write_text(json.dumps(qa,indent=2));assert qa['passed'],qa
print('PAVING_V17_INDEPENDENT_QA_PASS',qa['triangles'],qa['mesh_sha256'])
