...
## Current native34 outline correction

Current delivered mesh is the worn-outline refinement documented in `SELF_REVIEW_NATIVE_EDGES.md`: 114,550 triangles, 191,943 vertices, 426 closed stones, 1,134 broad edge recesses and 107 selected corner losses. The prior valid native34 package is frozen under `native34-preserved/`. Reproduce the current mesh with `wear_native_edges_v10.py`. All current triangles pass the positive-dot native convention, with zero degenerate or inverted top faces. SHA256 is `e9dace2039a9fb50ce028aa3b7cbff1f06893421dfa2d896e19c38f48d34c240`.

Actual neutral before/after previews were inspected before handoff; the parent also found the broad chipped arrises improved. Current status is READY_FOR_NATIVE36_REIMPORT, with full-scene acceptance pending the parent's new capture. Counts and SHA256 in the historical sections below describe their respective earlier stages.

## Native winding correction

Native candidate33 exposed a serious exporter error: almost all paving faces were culled. My previous QA asserted a negative cross-product/normal dot sign as "Unity clockwise" without comparing the working native mesh contract. That assertion was wrong, and the earlier winding PASS statements did not establish actual Unity compatibility.

Preserved the broken export, scripts, reports, and previews under `rejected-winding/` in both owned source/evidence locations. Corrected the JSON by reversing each triangle exactly once, leaving positions, normals, UVs, and colors byte-value-identical. The corrected JSON SHA256 is `f325ac982130a408eed81f2cdea0e9a4bf9b084bc11057188cf6fa5b44162c78`. Blender geometry and renders are unchanged because the defect was in the JSON winding conversion.

Both exporters now preserve triangle order under the proper axis rotation. The independent `validate_export.py --compare-rejected` asserts the same sign as actual working sources: current paving 96,060 positive / 0 negative; StoneV7 masonry 624 / 0; StoneV7 flagstone 896 / 0; TreeV10 90,768 / 0. All 43,626 triangles with supplied top-facing normals now have geometrically upward face normals. There are zero degenerate triangles. It also verifies every triangle changed exactly once and all four non-index arrays stayed identical.

Corrected static status: READY_FOR_NATIVE_REIMPORT. Actual native visibility requires the parent's candidate34 reimport/capture; I have not claimed that live verification passed. No Blender render or Unity edit was performed for this correction.
...
Final counts supersede the layout-stage counts below: 96,060 triangles, 131,884 exported vertices, 426 closed stone solids. Degenerate-triangle QA passed; the original winding check at this stage was incorrect and is superseded by the native correction above. Normals remain unit length, arrays finite, and the maximum top at -0.023654914 retains 19.65 mm water clearance. All eight frozen V10 files and thirteen original inputs remain hash-identical. The lower triangle count comes from replacing redundant radial cap rings with a purposeful fracture triangulation.
...
Verified at the layout stage: all 13 source files retain their SHA256; all numeric arrays are finite; all indices are valid; all source-authored stone solids are manifold; exported normals have length 0.9999998425 to 1.0000001481; independent JSON inspection finds zero degenerate triangles. The layout-stage winding checker used the wrong sign and its prior PASS was invalid for native Unity. The highest replacement point is Y=-0.02365394, 19.65 mm below the planned separate water surface at -0.004. The nominal top is the measured source median, -0.03176355.
