...
if '--skip-before' not in sys.argv and '--skip-preview' not in sys.argv:preview('before-native34-edges')
...
for idx,ob in enumerate(objects):
    ...
    def surface(pt):
        ...
        # A newly eroded corner can project beyond the old top cap and hit its
        # underside. Reject that height so the side band cannot fold below base.
        return max(hit.z,median_top-.040) if hit is not None else median_top
    ...
    faces=[]
    # The lower side band is buried in the foundation. Collapse its redundant
    # intermediate ring while keeping the visible arris and top contour exact.
    for lower,upper in ((0,2),(2,3)):
        for j in range(m):faces.append((lower*m+j,lower*m+(j+1)%m,upper*m+(j+1)%m,upper*m+j))
    ...
    check=bmesh.new();check.from_mesh(me)
    loose=[v for v in check.verts if not v.link_edges]
    if loose:bmesh.ops.delete(check,geom=loose,context='VERTS')
    bmesh.ops.remove_doubles(check,verts=list(check.verts),dist=.000001);bound=[e for e in check.edges if e.is_boundary]
...
if '--skip-preview' not in sys.argv:preview('after-native34-worn-edges')
...
report.update({'native_edge_refinement':'Candidate34-targeted broad uneven outline recesses; retained slab layout and internal cap fractures',
    'lower_band_optimization':'One redundant buried side ring removed, retaining visible upper arris construction. A 40mm cap-height floor rejects underside ray hits that otherwise fold the side band. Neutral after-native34-worn-edges previews show the broad outline changes before those localized corrections.',
...
validated=json.loads((OUT/'export-qa.json').read_text())['current']
report['normal_length_min_max']=validated['normal_length_min_max'];report['finite']=validated['finite'];report['valid_indices']=validated['valid_indices']
report['winding_correction']['applies_to']='Historical native34 baseline; current native-edge refinement changes geometry while preserving its positive-dot native convention.'
(OUT/'paving-v10-report.json').write_text(json.dumps(report,indent=2))
