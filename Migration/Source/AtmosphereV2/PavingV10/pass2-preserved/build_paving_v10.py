...
qa={'seed':SEED,'source_sha256':hashes,'source_preserved':{p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest()==h for p,h in hashes.items()},
    ...
    'export_basis':'Unity world space at identity. Blender (x,y,z) -> Unity (x,z,-y); reverse triangle winding; normals same rotation. colors = RGBA float array, constant per stone; uv = world XZ * .72.',
