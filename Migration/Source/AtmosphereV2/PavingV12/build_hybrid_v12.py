"""V12 source-matched 24 physical slabs over a captured stonebed. Blender 5.2.
...
rng=random.Random(120041)
prior=json.loads((V11/'paving-v11-mesh.json').read_text());oldmeta=json.loads((V11/'heightfield-metadata.json').read_text())
rects=oldmeta['coverage_rectangles_xz'];bounds=oldmeta['world_bounds'];PERIOD=(9.2,4.6)
sourcepaths=[V11/'paving-v11-mesh.json',V11/'heightfield-metadata.json',OUT/'source_seeds.json']+list(MAPS.glob('*.png'))
...
for g in guides:
 ...
 for j,(a,b) in enumerate(zip(raw,np.roll(raw,-1,axis=0))):
  ...
  steps=max(2,math.ceil(np.linalg.norm(edge)/.075))
 ...
 templates.append({'id':g['id'],'poly':poly,'center':poly.mean(0)})

# Actual native home camera projects the selection into lower-frame foreground.
a=.46;e=.72;right=np.array([math.cos(a),0,math.sin(a)]);up=np.array([math.sin(a)*math.sin(e),math.cos(e),-math.cos(a)*math.sin(e)])
focus=np.array([0,2.9,.7]);pxperm=1024/(2*(14.1/1.27))
def project(x,z):
 p=np.array([x,0,z])-focus;return [768+np.dot(p,right)*pxperm,512-np.dot(p,up)*pxperm]
candidates=[]
for tx in (-1,0):
 for tz in (0,1,2,3):
  for t in templates:
   shift=np.array([tx*PERIOD[0],tz*PERIOD[1]]);poly=t['poly']+shift;center=poly.mean(0);screen=project(*center)
   area=abs(sum(np.cross(poly[j],poly[(j+1)%len(poly)]) for j in range(len(poly))))*.5
   if 145<screen[0]<1320 and 550<screen[1]<995 and -7.8<center[0]<7.7 and center[1]>1:
    candidates.append({'template':t,'shift':shift,'poly':poly,'center':center,'screen':screen,'area':area})
# Prefer larger units but distribute selected boundaries across the lower view.
candidates.sort(key=lambda c:c['area'],reverse=True);selected=[]
for c in candidates:
 if sum(s['template']['id']==c['template']['id'] for s in selected)>=3:continue
 if any(np.linalg.norm(np.array(c['screen'])-np.array(s['screen']))<48 for s in selected):continue
 selected.append(c)
 if len(selected)==24:break
assert len(selected)==24,(len(selected),len(candidates))
...
for i,s in enumerate(selected):
 ...
 # Selected independent source units vary15-20% without a new regular grid.
 if i%3==0:scale=np.array([1.0 if i%2 else .84,.82 if i%2 else 1.0])
 ...
 level=[-.027,-.018,-.012,-.003,.003,-.009][i%6];tilt=np.array([rng.uniform(-.006,.006),rng.uniform(-.006,.006)])
 ...
 for ring in range(3):
  for j,p in enumerate(poly):
   direction=center-p;direction/=np.linalg.norm(direction)
   width=.065+.040*(.5+.5*math.sin(j/n*math.tau*2+i))
   q=p+(direction*width if ring==2 else 0)
   yy=-.115 if ring==0 else cap_y(*q)-(.021+.009*(.5+.5*math.sin(j/n*math.tau+i)) if ring==1 else 0)
   verts.append((q[0],-q[1],yy))
 # Stable radial interior rings retain scan residual without skinny-ngon
 # subdivision. Selected source polygons must be star-shaped about center.
 top_ring=[np.array([v[0],-v[1]]) for v in verts[2*n:3*n]]
 for fraction in (.68,.34):
  for q in top_ring:
   pt=center+(q-center)*fraction;verts.append((pt[0],-pt[1],cap_y(*pt)))
 topcenter=len(verts);verts.append((center[0],-center[1],cap_y(*center)))
 bottomcenter=len(verts);verts.append((center[0],-center[1],-.115))
 faces=[]
 for ring in range(2):
 ...
 for ring in (2,3):
  for j in range(n):faces.append((ring*n+j,ring*n+(j+1)%n,(ring+1)*n+(j+1)%n,(ring+1)*n+j))
 for j in range(n):
  faces.append((4*n+j,4*n+(j+1)%n,topcenter));faces.append(((j+1)%n,j,bottomcenter))
...
data={'id':'paving-v12-hybrid-scan-slabs','name':'PavingV12HybridScanSlabs','alreadyUnity':True,'positions':pos,'normals':norm,'uv':uv,'indices':indices,'colors':colors}
...
(OUT/'paving-v12-mesh.json').write_text(json.dumps(data,separators=(',',':')))
report={'hero_stones':stone_reports,'hero_count':len(stone_reports),'parts':parts,'triangles':len(indices)//3,'vertices':len(pos)//3,'coverage_rectangles_xz':rects,'water_y':-.004,'world_bounds':[[min(pos[a::3]) for a in range(3)],[max(pos[a::3]) for a in range(3)]],'source_sha256':hashes,'sources_preserved':{p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest()==h for p,h in hashes.items()},'mesh_sha256':hashlib.sha256((OUT/'paving-v12-mesh.json').read_bytes()).hexdigest(),'height_note':'Source40mm amplitude retained in bed; hero caps use independent art-directed planes, some above water, plus bounded2.8mm scan residual. Not vertical measurements.','uv_note':'Bed worldXZ/[9.2,4.6]; each hero uses its documented inverse source attachment transform. Use exported UV; never shader world UV on hero stones.','limitations':['AO-guided contours are manually identified source regions, not guaranteed semantic auto-segmentation.','Fine source cracks and full-map repeated courses remain in continuous bed.','Root integrates native roughness, illumination and independent pools.']}
(OUT/'hybrid-metadata.json').write_text(json.dumps(report,indent=2))
oldp=np.array(prior['positions']).reshape(-1,3);oldob=make_object('BEFORE V11 preserved',np.stack((oldp[:,0],-oldp[:,2],oldp[:,1]),1).tolist(),np.array(prior['indices']).reshape(-1,3).tolist());uvlay=oldob.data.uv_layers.new(name='UVMap');olduv=np.array(prior['uv']).reshape(-1,2)
...
scene=bpy.context.scene;scene.render.engine='CYCLES';scene.cycles.samples=24;scene.cycles.use_denoising=True;scene.world.color=(.10,.10,.10)
...
for view,target,width in [('native-ground',(0,-.7,2.9),33.3070866),('close',(0,-8,0),13.0)]:
 ...
 for kind in ['before-v11-neutral','after-v12-neutral','after-v12-textured']:
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'paving-v12.blend'))
print('PAVING_V12_DONE',report['triangles'],report['vertices'],report['mesh_sha256'])
