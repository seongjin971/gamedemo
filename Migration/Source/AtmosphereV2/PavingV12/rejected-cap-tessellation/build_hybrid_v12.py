...
for i,s in enumerate(selected):
 ...
 if i%3==0:scale=np.array([1.16 if i%2 else .84,1.04 if i%2 else 1.08])
 ...
 for ring in range(3):
  for j,p in enumerate(poly):
   ...
   width=.055+.025*(.5+.5*math.sin(j/n*math.tau*2+i))
 ...
 faces.append(tuple(range(n-1,-1,-1)));faces.append(tuple(range(2*n,3*n)))
 ob=make_object(f'HeroScanSlab_{i:02d}_{s["template"]["id"]}',verts,faces);me=ob.data
 bm=bmesh.new();bm.from_mesh(me);bm.faces.ensure_lookup_table();topface=bm.faces[-1];topverts=set(topface.verts)
 bmesh.ops.triangulate(bm,faces=[topface]);edges=[ed for ed in bm.edges if all(v in topverts for v in ed.verts)]
 bmesh.ops.subdivide_edges(bm,edges=edges,cuts=3,use_grid_fill=True)
 for v in bm.verts:
  if v.co.z>-.07:v.co.z=cap_y(v.co.x,-v.co.y) if v.co.z>level-.009 else v.co.z
