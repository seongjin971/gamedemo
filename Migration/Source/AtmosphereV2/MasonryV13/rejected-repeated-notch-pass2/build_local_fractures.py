...
for node in bridge['nodes']:
    ...
    for j,(ix,inst) in enumerate(selected):
        ...
        if (node['id'][:8],ix) in chosen:
            ...
            # Open mid-arris fracture away from inherited top-corner damage.
            # Maximum inward depth is <=0.22 of the normalized block; the
            # finite vertical footprint does not span the entire arris.
            side=-side;z=-.11+.047*(k%5)
            cut(ob,[(side*.506,-.506,z-.22),(side*.506,-.506,z+.22),
                    (side*.29,-.506,z-.12),(side*.33,-.506,z+.16),
                    (side*.506,-.33,z-.15),(side*.506,-.36,z+.17),
                    (side*.39,-.414,z-.013),(side*.425,-.398,z+.045)],'Broad finite side-face fracture')
