...
for row in range(10):
    ...
    for col in range(9):
        ...
        damaged=(col+row*2)%3==1
        if damaged:
            cx=xmin+(xmax-xmin)*rng.uniform(.18,.82);half=rng.uniform(.10,.19);depth=rng.uniform(.026,.049);down=rng.uniform(.022,.045)
        ...
        if (row*11+col)%6==2:
            # Unequal broad open corner fracture, still less than 1/4 block depth.
            side=-1 if row%2 else 1;edge=xmin if side<0 else xmax
            cut(ob,[(edge+side*.025,-front-.025,ytop+.03),(edge-side*.11,-front-.025,ytop+.03),(edge+side*.025,-front+.07,ytop+.03),
                    (edge+side*.025,-front-.025,ytop-.06),(edge-side*.045,-front-.025,ytop-.025),(edge+side*.025,-front+.045,ytop-.025)])
        ...
        bmesh.ops.subdivide_edges(bm,edges=long,cuts=6,use_grid_fill=True)
        ...
        for v in bm.verts:
            ...
            if p.z>ytop-.012:
                ...
                p.z-=.0055*max(0,1-u*u)*max(0,1-w*w)*(1+.4*math.sin(p.x*13+row))
                # Nonperiodic broad worn flats and unequal front-arris interruptions.
                along=(p.x-(xmin+(xmax-xmin)*(.31 if row%2 else .64)))/.19
                edge=max(0,1-abs(p.y+front)/.075)
                p.z-=.011*max(0,1-abs(along))*edge
                p.z-=.0028*(.5+.5*math.sin(p.x*17.4+row*.83))*max(0,1-abs(w))
            if abs(p.y+front)<.015:
                u=(p.x-(xmin+(xmax-xmin)*(.35 if col%2 else .65)))/.22;v1=(p.z-(bottom+.16))/.12
                p.y+=(.009+.002*math.sin(row*1.9+col*.7))*max(0,1-u*u-v1*v1)
        ...
        me.materials.clear();me.materials.append(clay)
        # Hard split planes keep chisel losses readable; no inflated smooth corners.
        for p in me.polygons:p.use_smooth=True;p.material_index=0
        bpy.context.view_layer.objects.active=ob
        sharp=ob.modifiers.new('Preserve hard fracture planes','EDGE_SPLIT');sharp.split_angle=.27
        bpy.ops.object.modifier_apply(modifier=sharp.name)
...
pos=[];norm=[];uv=[];colors=[];inds=[];qa=[];dedup={}
for ob in objects:
    me=ob.data;me.calc_loop_triangles();bm=bmesh.new();bm.from_mesh(me);bmesh.ops.remove_doubles(bm,verts=list(bm.verts),dist=1e-7);nonmanifold=sum(not e.is_manifold for e in bm.edges);bm.free()
    ...
    for tri in me.loop_triangles:
        for li in tri.loops:
            ...
            key=(p.x,p.z,-p.y,n.x,n.z,-n.y,p.x,-p.y,ob['tone'])
            if key not in dedup:
                dedup[key]=len(pos)//3;pos.extend(key[:3]);norm.extend(key[3:6]);uv.extend(key[6:8]);colors.extend((ob['tone'],)*3+(1,))
