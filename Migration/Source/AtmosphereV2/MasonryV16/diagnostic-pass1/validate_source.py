...
for file in sorted(OUT.glob('*-mesh.json'))+sorted(OUT.glob('hero-v16-*.json')):
 ...
 reports.append({'file':file.name,'sha256':hashlib.sha256(file.read_bytes()).hexdigest(),'triangles':len(idx),'vertices':len(p),'bounds':[p.min(axis=0).tolist(),p.max(axis=0).tolist()],'minimum_normal_dot':float(dots.min()),'parts':partreports})
