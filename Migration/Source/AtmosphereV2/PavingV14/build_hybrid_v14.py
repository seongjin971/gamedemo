"""V14 contiguous source-matched foreground slabs over a captured stonebed. Blender 5.2.
...
rng=random.Random(140047)
prior=json.loads((OUT.parent/'PavingV13/paving-v13-mesh.json').read_text());oldmeta=json.loads((V11/'heightfield-metadata.json').read_text())
rects=oldmeta['coverage_rectangles_xz'];bounds=oldmeta['world_bounds'];PERIOD=(12.4,6.2)
sourcepaths=[OUT.parent/'PavingV13/paving-v13-mesh.json',V11/'paving-v11-mesh.json',V11/'heightfield-metadata.json',OUT/'source_seeds.json']+list(MAPS.glob('*.png'))
...
assert 450<=len(selected)<=550,len(selected)
...
for i,s in enumerate(selected):
 ...
 for ring in range(3):
  for j,p in enumerate(poly):
   ...
   depth=.75*((.013+.006*(.5+.5*math.sin(j/n*math.tau+i))) if subordinate else (.021+.009*(.5+.5*math.sin(j/n*math.tau+i))))
 ...
 # A bounded residual sampled onto a very thin ear triangle can create an
 # unbounded apparent slope. Condition only those cap residual vertices,
 # relative to this stone's own authored plane, before normal averaging.
 # The 12-degree cone guarantees mutually compatible area-weighted normals;
 # geometric/winding gates below remain unchanged.
 cap_face_list=faces[2*n:2*n+len(captris)*3]
 original_cap_y={k:verts[k][2] for f in cap_face_list for k in f}
 plane_normal=Vector((-tilt[0],tilt[1],1.)).normalized();limited=set()
 for conditioning_iteration in range(24):
  steep=set()
  for f in cap_face_list:
   aa,bb,cc=[Vector(verts[k]) for k in f];cr=(bb-aa).cross(cc-aa)
   if cr.z<0:cr=-cr
   if cr.normalized().dot(plane_normal)<math.cos(math.radians(12)):steep.update(f)
  if not steep:break
  for k in steep:
   xx,yy,zz=verts[k];plane_y=level+np.dot(np.array([xx,-yy])-center,tilt)
   verts[k]=(xx,yy,float(plane_y+(zz-plane_y)*.5));limited.add(k)
 else:raise AssertionError(('cap residual conditioning did not converge',s['template']['id']))
 for j in range(n):
  xx,yy,zz=verts[n+j];verts[n+j]=(xx,yy,zz+verts[2*n+j][2]-original_cap_y[2*n+j])
 residual_conditioning={'limited_vertices':len(limited),'iterations':conditioning_iteration,'max_height_adjustment_m':max((abs(verts[k][2]-original_cap_y[k]) for k in limited),default=0.),'cap_slope_cone_degrees_from_own_plane':12}
 ...
 # Smooth only the cap's internal triangulation, within this body. The
 ...
 smoothing_report={'cap_faces':len(capfaces),'method':'per-body cap-only area-weighted geometry normals; hard perimeter/bevel/sides','cap_face_marker':'cap_surface','residual_conditioning':residual_conditioning}
 objects.append(ob)
 stone_reports.append({'name':ob.name,'source_id':s['template']['id'],'subordinate':subordinate,'cap_smoothing':smoothing_report,'source_polygon_world_xz':source.tolist(),'polygon_world_xz':poly.tolist(),'cap_polygon_world_xz':top_ring.tolist(),'cap_area_m2':area(top_ring),'inset_scale':inset_scale,'center':center.tolist(),'scale':scale.tolist(),'plane_level':level,'plane_slope_xz':tilt.tolist(),'source_uv_formula':'((worldXZ-center)/scale+center)/[12.4,6.2]','closed_nonmanifold_edges':nonmanifold,'screen_center':s['screen'],'roughness_bias_suggestion':round(rng.uniform(-.06,.06),3)})
...
x=xs[used%len(xs)];z=zs[used//len(xs)];y=-.0236+.04*(sample(height,x/12.4,z/6.2)-1)
for s in selected:
...
for loop in bed.data.loops:
 co=bed.data.vertices[loop.vertex_index].co;uvlay.data[loop.index].uv=(co.x/12.4,-co.y/6.2)
...
data={'id':'paving-v14-hybrid-scan-slabs','name':'PavingV14HybridScanSlabs','alreadyUnity':True,'positions':pos,'normals':norm,'uv':uv,'indices':indices,'colors':colors}
assert len(indices)//3<=141000,len(indices)//3
...
if not np.all(dots>0):
 bad=[]
 for triid in np.nonzero(dots<=0)[0]:
  oi=next(k for k,part in enumerate(parts) if part['index_start']<=triid*3<part['index_start']+part['index_count'])
  ob=objects[oi];local=(triid*3-parts[oi]['index_start'])//3;triangle=ob.data.loop_triangles[local]
  capflag=ob.data.attributes.get('cap_surface')
  bad.append({'triangle':int(triid),'part':ob.name,'is_cap':bool(capflag and capflag.data[triangle.polygon_index].value),'dot':float(dots[triid]),'positions':pp[ii[triid]].tolist(),'normals':nn[ii[triid]].tolist()})
 (OUT/'normal-preflight-rejection.json').write_text(json.dumps(bad,indent=2))
...
(OUT/'paving-v14-mesh.json').write_text(json.dumps(data,separators=(',',':')))
report={'period_m':[12.4,6.2],'source_nominal_period_m':[2.3,1.15],'additional_art_scale_from_v13':12.4/9.2,'total_art_scale_from_source':12.4/2.3,'shoulder_drop_scale_from_v13':.75,'hero_stones':stone_reports,'hero_count':len(stone_reports),'source_contour_count':len(templates),'skipped_contours':skipped,'foreground_body_area_m2':sum(s['area'] for s in selected),'foreground_area_m2':(rects[0][1]-rects[0][0])*(rects[0][3]-rects[0][2]),'parts':parts,'triangles':len(indices)//3,'vertices':len(pos)//3,'coverage_rectangles_xz':rects,'water_y':-.004,'world_bounds':[[min(pos[a::3]) for a in range(3)],[max(pos[a::3]) for a in range(3)]],'source_sha256':hashes,'sources_preserved':{p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest()==h for p,h in hashes.items()},'mesh_sha256':hashlib.sha256((OUT/'paving-v14-mesh.json').read_bytes()).hexdigest(),'height_note':'Photographed horizontal period enlarged with aligned UV/height; additional art scale, not source metric dimensions. Source40mm amplitude retained in bed; hero caps use independent art-directed planes, some above water, plus bounded2.8mm scan residual. Not vertical measurements.','uv_note':'Bed worldXZ/[12.4,6.2]; each hero uses its documented inverse source attachment transform. Use exported UV; never shader world UV on hero stones.','limitations':['AO-guided contours are manually identified source regions, not guaranteed semantic auto-segmentation.','Fine source cracks and full-map repeated courses remain in continuous bed.','Root integrates native roughness, illumination and independent pools.']}
(OUT/'hybrid-metadata.json').write_text(json.dumps(report,indent=2))
oldp=np.array(prior['positions']).reshape(-1,3);oldob=make_object('BEFORE V13 preserved',np.stack((oldp[:,0],-oldp[:,2],oldp[:,1]),1).tolist(),np.array(prior['indices']).reshape(-1,3).tolist());uvlay=oldob.data.uv_layers.new(name='UVMap');olduv=np.array(prior['uv']).reshape(-1,2)
...
if '--geometry-only' in sys.argv:
 ...
 bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'paving-v14.blend'))
 print('PAVING_V14_GEOMETRY_DONE',report['triangles'],report['vertices'],report['mesh_sha256'])
...
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'paving-v14.blend'))
print('PAVING_V14_DONE',report['triangles'],report['vertices'],report['mesh_sha256'])
