...
import json,math,hashlib,collections,runpy,struct
...
g=json.loads((OUT/'paving-v14-mesh.json').read_text());m=json.loads((OUT/'hybrid-metadata.json').read_text())
...
def face_cross(pts):
 a,b,c=pts;x,y,z=(b[k]-a[k] for k in range(3));u,v,w=(c[k]-a[k] for k in range(3))
...
for part in m['parts']:
 ...
 if part['name'].startswith('Hero'):
  ...
  info['source_uv_max_error']=max(max(abs(uv[i*2]-((p[i*3]-cx)/sx+cx)/12.4),abs(uv[i*2+1]-((p[i*3+2]-cz)/sz+cz)/6.2)) for i in used)
  ...
  # Metadata contour was authored in Python doubles; actual Blender vertices
  # are float32. Classify against that same float32 contour representation,
  # retaining the original membership tolerance and strict normal/area gates.
  cap_poly=[[struct.unpack('f',struct.pack('f',v))[0] for v in pt] for pt in stone['cap_polygon_world_xz']]
  membership={i:p[3*i+1]>-.08 and in_cap(p[3*i],p[3*i+2],cap_poly) for i in used}
  sums=collections.defaultdict(lambda:[0.,0.,0.]);groups=collections.defaultdict(set);noncap_cos=1.;cap_count=0
  for k in range(0,len(inds),3):
   tri=inds[k:k+3];cr=face_cross([p[3*i:3*i+3] for i in tri])
   if cr[1]>0 and all(membership[i] for i in tri):
    cap_count+=1
    for i in tri:
     key=tuple(p[3*i:3*i+3])+tuple(uv[2*i:2*i+2]);groups[key].add(i)
     for a in range(3):sums[key][a]+=cr[a]
   else:
    expected=unit(cr)
    noncap_cos=min(noncap_cos,min(sum(expected[a]*n[3*i+a] for a in range(3)) for i in tri))
  cap_cos=1.
  for key,ids in groups.items():
   expected=unit(sums[key]);cap_cos=min(cap_cos,min(sum(expected[a]*n[3*i+a] for a in range(3)) for i in ids))
  info['cap_area_weighted_normal_min_cosine']=cap_cos;info['noncap_flat_normal_min_cosine']=noncap_cos;info['cap_triangles']=cap_count
  assert cap_count==stone['cap_smoothing']['cap_faces'],('cap classification mismatch',info)
  assert cap_cos>1-1e-6 and noncap_cos>1-1e-6,('cap smoothing or hard-side preservation failed',info)
 else:
  info['source_uv_max_error']=max(max(abs(uv[i*2]-p[i*3]/12.4),abs(uv[i*2+1]-p[i*3+2]/6.2)) for i in used)
  assert info['source_uv_max_error']<1e-6
...
qa['mesh_sha256']=hashlib.sha256((OUT/'paving-v14-mesh.json').read_bytes()).hexdigest()
...
assert qa['triangles']<=141000 and qa['positive_dot']==qa['triangles'] and not qa['negative_dot'] and not qa['degenerate']
...
assert 450<=qa['hero_count']<=550 and qa['exposed_stone_count']>0 and qa['fully_submerged_stone_count']>0
