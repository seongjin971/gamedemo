...
public static partial class WorldVisuals {
 ...
 public static void Compose(GameObject collision,WorldEnvironment world,string generated){
  ...
  var camera=Camera.main.GetUniversalAdditionalCameraData();camera.requiresDepthTexture=true;camera.requiresColorTexture=true;camera.antialiasing=AntialiasingMode.SubpixelMorphologicalAntiAliasing;camera.antialiasingQuality=AntialiasingQuality.High;
  ...
  foreach(var m in materials.Values.ToArray()){
   ...
   if(m.name.StartsWith("Abbey_CW_Chapel")){m.SetTexture("_BaseMap",stoneTexture);m.SetTextureScale("_BaseMap",Vector2.one);m.SetTextureOffset("_BaseMap",Vector2.zero);m.color=new Color(.44f,.48f,.45f);m.SetFloat("_Smoothness",.35f);}
 ...
 static void Rock(Transform p,float x,float d,float width,float height,float depth,float yaw,bool snowy=false){
  var pos=G(x,d,height*.7f);var scale=new Vector3(width,height,depth);
 ...
 static void River(Transform p,WorldEnvironment world){
  ...
  Tree(p,7.7f,35,1.3f,false,false,0);Tree(p,-9.3f,13,1.0f,false,false,0);
 }
 static void Paving(Transform p,float d,float scale,float wet){
  var go=Model(p,"VisualF/CW_BroadPaving_"+(1+stoneSerial%3),WorldLayout.Point(d)+Vector3.up*.01f,new Vector3(scale,1,scale),stoneSerial%2*180);
  foreach(var mf in go.GetComponentsInChildren<MeshFilter>()){
   ...
   var mats=renderer.sharedMaterials;for(int k=0;k<mats.Length;k++)if(mats[k].name.StartsWith("Wet_")){
    ...
    if(!materials.TryGetValue(key,out var replacement)){replacement=new Material(Shader.Find(wet>.2f?"Vesper/Linear/WetStone":"Universal Render Pipeline/Lit")){name=key};replacement.SetTexture("_BaseMap",stoneTexture);replacement.SetColor("_BaseColor",wet>.2f?new Color(.73f,.77f,.75f):new Color(.84f,.83f,.75f));replacement.SetFloat("_Wetness",wet);replacement.SetFloat("_ReflectionStrength",.18f);replacement.SetFloat("_Smoothness",.2f);AssetDatabase.CreateAsset(replacement,output+"/"+key+".mat");materials[key]=replacement;}
    mats[k]=replacement;
 ...
 static void Path(Transform p){
  for(float d=2;d<=300;d+=3.7f){if(d>16&&d<34)continue;float wet=WorldLayout.Weights(d).y;
   bool continuous=d<48||(d>124&&d<179);if(!continuous&&((int)(d*10)%3!=0))continue;
   if(d>253)continue;Paving(p,d,continuous?.97f:R(.55f,.85f),wet);
  }
 }
 static void Abbey(Transform p,WorldEnvironment world){
  ...
  foreach(float d in new[]{146f,150f,154f})Model(p,"VisualF/CW_BroadPaving_"+(d==150?2:1),new Vector3(7.7f,y,-d),new Vector3(.85f,1,.95f),0);
 ...
 static void Vegetation(Transform p){
  for(float d=-8;d<318;d+=6.3f)foreach(float side in new[]{-1f,1f}){
   ...
   Tree(p,off,td,R(.8f,1.22f),fir,fir&&td>205,R(0,360));
  ...
  for(int i=0;i<1600;i++){
   float d=R(-9,262),x=(i%2==0?-1:1)*R(2.4f,15);if(WorldLayout.River(x,d)||ChapelFootprint(x,d)||d>10&&d<41)continue;
   if(d>208&&random.NextDouble()<S(205,260,d))continue;
   float clump=.5f+.5f*Mathf.Sin(d*.51f+x*.68f);if(clump<.27f)continue;
   Model(p,"CW_RiverGrass",G(x,d,.05f),new Vector3(R(.38f,.9f),R(.32f,.73f),R(.5f,1.05f)),R(0,360));
  }
