...
public static partial class WorldVisuals {
 ...
 static GameObject Model(Transform parent,string file,Vector3 pos,Vector3 scale,float yaw,bool blocker=false,bool hf=false){
  ...
  if(file.StartsWith("VisualF/")||file.StartsWith("L02Details/"))path=LinearBuild.Root+"/Art/EnvironmentKit/"+file+".fbx";var asset=AssetDatabase.LoadAssetAtPath<GameObject>(path);if(!asset){throw new Exception("Missing required model "+path);}
