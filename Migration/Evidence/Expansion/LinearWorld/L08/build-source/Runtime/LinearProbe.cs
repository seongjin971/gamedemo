...
public sealed class LinearProbe:MonoBehaviour {
 ...
 [Serializable]public class Report{public string mode,device,note;public int width,height,failures,fixtures,frames,clicks,drags,zooms,resets,rejected,safetyStops,audioSources;public float distance;public Vector3 position;public string[] checks,errors;public Perf[] performance;}
 ...
 IEnumerator Place(float d){motor.Stop();motor.transform.position=WorldLayout.Point(d);motor.transform.rotation=Quaternion.Euler(0,180,0);fixtures++;yield return new WaitForSecondsRealtime(4);}
 IEnumerator Pictures(){
  if(Array.IndexOf(Environment.GetCommandLineArgs(),"-linearDiagnostics")>=0){
   var materials=Resources.FindObjectsOfTypeAll<Material>().Where(m=>m.HasProperty("_Debug")).ToArray();
   foreach(float d in new[]{25f,150f}){yield return Place(d);for(int debug=0;debug<=4;debug++){foreach(var mat in materials)mat.SetFloat("_Debug",debug);yield return null;yield return Snap("diagnostic-"+d+"-"+debug);}foreach(var mat in materials)mat.SetFloat("_Debug",0);}yield break;
  }
  foreach(float d in new[]{25f,85f,150f,215f,275f}){yield return Place(d);Check(Vector3.Distance(motor.transform.position,WorldLayout.Point(d))<.01f,"Capture fixture "+d+" intact");yield return Snap("hero-"+d);foreach(float delta in new[]{-5f,5f}){yield return Place(d+delta);yield return Snap("near-"+(d+delta));}}
 }
