...
public static class LinearRoadBuild {
 ...
 static string CollisionStamp(Scene s)=>string.Join("\n",All<Collider>(s).Select(c=>Pose(c.transform)+"|"+c.GetType().FullName+"|"+c.enabled+"|"+c.isTrigger+"|"+c.bounds.ToString("F5")+"|"+(c is MeshCollider m?AssetPath(m.sharedMesh):EditorJsonUtility.ToJson(c))).OrderBy(x=>x));
