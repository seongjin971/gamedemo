...
public sealed class WorldEnvironment:MonoBehaviour {
 ...
 void Update(){
  ...
  if(sun){sun.color=new Color(1,.93f,.77f)*a+new Color(.72f,.80f,.89f)*b+new Color(.92f,.95f,1)*c;sun.intensity=1.65f*a+.72f*b+1.1f*c;}
  if(sun)sun.shadowStrength=.9f*a+.65f*b+.65f*c;
  RenderSettings.ambientSkyColor=new Color(.55f,.65f,.75f)*a+new Color(.38f,.47f,.55f)*b+new Color(.64f,.70f,.79f)*c;
  RenderSettings.ambientEquatorColor=new Color(.37f,.43f,.38f)*a+new Color(.25f,.31f,.35f)*b+new Color(.49f,.53f,.60f)*c;
  RenderSettings.ambientGroundColor=new Color(.20f,.23f,.16f)*a+new Color(.16f,.20f,.21f)*b+new Color(.30f,.35f,.42f)*c;
  RenderSettings.fog=true;RenderSettings.fogMode=FogMode.ExponentialSquared;
  RenderSettings.fogColor=new Color(.63f,.73f,.74f)*a+new Color(.40f,.49f,.55f)*b+new Color(.72f,.77f,.81f)*c;
  ...
  if(reflection)reflection.enabled=Progress<65;
  if(ruinsReflection)ruinsReflection.enabled=Progress>110&&Progress<185;
