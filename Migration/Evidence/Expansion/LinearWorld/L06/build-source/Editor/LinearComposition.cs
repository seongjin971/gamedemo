...
public static partial class WorldVisuals {
 ...
 public static void Compose(GameObject collision,WorldEnvironment world,string generated){
  ...
  foreach(var m in materials.Values.ToArray()){
   ...
   if(m.shader.name=="Vesper/Linear/WetStone"){m.SetTexture("_WearMap",atlas);m.SetTexture("_SnowMap",snowTexture);}
 ...
 static void GroundMaterial(GameObject root){
  foreach(var r in root.GetComponentsInChildren<MeshRenderer>().Where(r=>r.name.StartsWith("Linear terrain"))){
   var mat=r.sharedMaterial;mat.shader=Shader.Find("Vesper/Linear/Ground");mat.SetTexture("_BaseMap",atlas);mat.SetTexture("_MossMap",Texture(Art+"/Textures/MossSurface.png"));mat.SetTexture("_SnowMap",snowTexture);mat.SetTexture("_PackedMap",Texture(Art+"/Textures/MaterialTexturesV2/PackedAlpineSnow.png"));mat.SetTexture("_StoneMap",stoneTexture);mat.SetFloat("_DetailScale",.23f);mat.SetFloat("_NormalStrength",.21f);
   var mesh=r.GetComponent<MeshFilter>().sharedMesh;var v=mesh.vertices;var normals=mesh.normals;var colors=new Color[v.Length];
   for(int i=0;i<v.Length;i++){float d=-v[i].z,off=Mathf.Abs(v[i].x)+Mathf.Sin(d*.67f)*.13f+Mathf.Sin(d*1.8f)*.08f;float path=1-S(1.2f,3.3f,off);float frost=WorldLayout.SnowCover(v[i].x,d);float snow=frost*S(.35f,.95f,normals[i].y);snow*=Mathf.Lerp(1,Mathf.Lerp(.85f,1,S(228,252,d)),path);colors[i]=new Color(path,snow,Mathf.Clamp01((1-normals[i].y)*3.2f),WorldLayout.Weights(d).y);}
 ...
 static void Paving(Transform p,float d,float scale,float wet,bool scattered=false){
  var go=Model(p,(scattered?"L04Details/LinearIrregular_":"L02Details/LinearPaving_")+(1+stoneSerial%3),WorldLayout.Point(d)+Vector3.up*.015f,new Vector3(scale,1,1),stoneSerial%2*180);
  foreach(var mf in go.GetComponentsInChildren<MeshFilter>()){
   var mesh=UnityEngine.Object.Instantiate(mf.sharedMesh);var v=mesh.vertices;
   for(int i=0;i<v.Length;i++){var w=mf.transform.TransformPoint(v[i]);float localHeight=w.y-WorldLayout.Level(d);if(localHeight>-.075f)localHeight=Mathf.Max(localHeight,.014f);w.y=WorldLayout.Height(w.x,-w.z)+localHeight;v[i]=mf.transform.InverseTransformPoint(w);}
   ...
   var mats=renderer.sharedMaterials;for(int k=0;k<mats.Length;k++)if(mats[k].name.StartsWith("Wet_")){
    ...
    replacement.SetFloat("_PathWear",1);mats[k]=replacement;
   }renderer.sharedMaterials=mats;
  }
 }
 static void Path(Transform p){
  for(float d=2;d<=300;d+=4f){if(d>16&&d<34)continue;float wet=WorldLayout.Weights(d).y;
   bool continuous=d<48||(d>124&&d<179);
   if(d>287)continue;float width=continuous?.84f:R(.85f,1.0f);
   // Stonework disappears beneath accumulated snow; the underlying packed trail continues.
   // Consume the same random values as L05 so this change does not reshuffle vegetation.
   if(d>=238)continue;
   width*=Mathf.Lerp(1,.38f,S(214,238,d));Paving(p,d,width,wet,!continuous);
  }
 }
 ...
 static void Vegetation(Transform p){
  for(float d=-8;d<318;d+=6.3f)foreach(float side in new[]{-1f,1f}){
   ...
   float td=d+R(-1.8f,1.8f),off=side*R(side<0?7.5f:5.2f,side<0?10.3f:9.3f);bool fir=td>230||(td>194&&random.NextDouble()<(td-188)/45);
   if(!(side<0&&td>209&&td<230))Tree(p,off,td,fir?R(.8f,1.22f):R(1.05f,1.62f),fir,fir&&td>205,R(0,360));
  ...
  for(int i=0;i<4400;i++){
   float d=R(-9,262),x=(i%2==0?-1:1)*R(2.15f,13);if(WorldLayout.River(x,d)||(ChapelFootprint(x,d)&&x>4&&d>145&&d<157)||d>10&&d<41)continue;
  ...
  Rock(p,-6.1f,219,1.18f,1.22f,1.3f,23,true);Rock(p,-7.4f,228,1.2f,.98f,1.3f,37,true);
  Rock(p,7.2f,278,1.38f,1.43f,1.60f,192,true);
 }
}
}
