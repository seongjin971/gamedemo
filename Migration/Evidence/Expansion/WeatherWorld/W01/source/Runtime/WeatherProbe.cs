...
public sealed class WeatherProbe:MonoBehaviour {
 ...
 IEnumerator Pictures(){env.FlashOverride=0;foreach(float d in new[]{25f,60,84,104,122,150,198}){yield return Place(d);yield return Snap("weather-"+d);}env.FlashOverride=1;env.StrongOverride=false;yield return new WaitForSecondsRealtime(.1f);yield return Snap("abbey-distant-flash");env.StrongOverride=true;yield return new WaitForSecondsRealtime(.1f);yield return Snap("abbey-strong-flash");env.FlashOverride=0;foreach(float d in new[]{258f,282,323}){yield return Place(d);yield return Snap("weather-"+d);}}
