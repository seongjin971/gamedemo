...
public static class WeatherBuild {
 ...
 static void ConfigureSnow(ParticleSystem ps,int kind){
  ps.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);var main=ps.main;main.maxParticles=kind==0?3200:kind==1?600:80;main.startLifetime=kind==2?3.2f:4.5f;main.startSize=kind==0?new ParticleSystem.MinMaxCurve(.018f,.055f):kind==1?new ParticleSystem.MinMaxCurve(.055f,.105f):new ParticleSystem.MinMaxCurve(2.5f,5.5f);main.startColor=new Color(.78f,.86f,1,kind==2?.20f:.72f);
  var shape=ps.shape;shape.scale=kind==2?new Vector3(16,.4f,26):new Vector3(28,4,32);
  var velocity=ps.velocityOverLifetime;velocity.x=kind==2?6:7.6f;velocity.y=kind==2?.02f:-2.8f;velocity.z=1.5f;
  ...
  var renderer=ps.GetComponent<ParticleSystemRenderer>();renderer.renderMode=kind==2?ParticleSystemRenderMode.HorizontalBillboard:ParticleSystemRenderMode.Stretch;renderer.velocityScale=kind==2?0:.009f;renderer.lengthScale=kind==2?1:1.15f;
  var mat=new Material(renderer.sharedMaterial){name=ps.name};mat.SetFloat("_Kind",kind==2?3:1);mat.SetColor("_BaseColor",new Color(.73f,.83f,1,kind==2?.26f:.88f));Save(mat,"SnowMaterial");renderer.sharedMaterial=mat;
