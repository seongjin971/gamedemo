...
public sealed class WorldEnvironment:MonoBehaviour {
 ...
 void Update(){
  ...
  float nightScale=Mathf.Lerp(1,.55f,night);
  Shader.SetGlobalVector("_WeatherFlashState",new Vector4(Flash,StrongFlash?1:0,Progress,night));
  if(sun){sun.color=Color.Lerp(sunColor,new Color(.72f,.84f,1),Flash*.55f);sun.intensity=Mathf.Lerp(1.65f,.72f,dark)*nightScale+distant*.10f+broad*1.15f;sun.shadowStrength=Mathf.Lerp(.9f,.65f,dark);}
  ...
  if(grade)grade.postExposure.value=Mathf.Lerp(.30f,.15f,dark)-night;
  ...
  WeatherAt(snow,player.transform.position+new Vector3(-11,8,0),1450*night*gust);
  WeatherAt(nearSnow,player.transform.position+new Vector3(-12,9,1),360*night*gust);
