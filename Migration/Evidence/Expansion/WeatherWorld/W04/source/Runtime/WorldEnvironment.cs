...
public sealed class WorldEnvironment:MonoBehaviour {
 ...
 public ParticleSystem rain,snow,spindrift,nearSnow,leaves;
 ...
 void Update(){
  ...
  if(sun){sun.color=Color.Lerp(sunColor,new Color(.52f,.71f,1),Flash*.8f);sun.intensity=Mathf.Lerp(1.65f,.72f,dark)*Mathf.Lerp(1,.30f,night)+distant*.10f+broad*.85f;sun.shadowStrength=Mathf.Lerp(.9f,.65f,dark)*Mathf.Lerp(1,.45f,night);}
  ...
  if(grade){grade.postExposure.value=Mathf.Lerp(.30f,.15f,dark)-night;grade.saturation.value=-5-16*dark*(1-rainWeight)*(1-night);}
  ...
  WeatherAt(snow,player.transform.position+new Vector3(-8,11,0),1450*night*gust);
  WeatherAt(nearSnow,player.transform.position+new Vector3(-9,12,1),300*night*gust);
