...
public static class WeatherBuild {
 ...
 static void ConfigureSnow(ParticleSystem ps,int kind){
  ...
  var renderer=ps.GetComponent<ParticleSystemRenderer>();renderer.renderMode=kind==2?ParticleSystemRenderMode.HorizontalBillboard:ParticleSystemRenderMode.Stretch;renderer.velocityScale=kind==2?0:kind==0?.009f:.015f;renderer.lengthScale=kind==2?1:kind==0?1.2f:2.2f;
  var mat=new Material(renderer.sharedMaterial){name=ps.name};mat.SetFloat("_Kind",kind==2?3:1);mat.SetColor("_BaseColor",new Color(.60f,.74f,.94f,kind==2?.65f:.8f));Save(mat,"SnowMaterial");renderer.sharedMaterial=mat;
 ...
 public static void Refine(){try{
  ...
  var profile=Object.Instantiate(world.volume.sharedProfile);profile.components=world.volume.sharedProfile.components.Select(c=>Object.Instantiate(c)).ToList();Save(profile,"WeatherGrade");foreach(var c in profile.components)AssetDatabase.AddObjectToAsset(c,profile);world.volume.sharedProfile=profile;
  ConfigureSnow(world.snow,0);ConfigureSnow(world.nearSnow,1);ConfigureSnow(world.spindrift,2);ConfigureLeaves(world);
