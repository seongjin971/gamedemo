...
public sealed class WeatherW07Probe:MonoBehaviour {
 ...
 [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]static void Install(){var a=Environment.GetCommandLineArgs();foreach(string key in new[]{"-w07Capture","-w07Traverse","-w07Performance"}){int i=Array.IndexOf(a,key);if(i<0||i+1>=a.Length)continue;var p=new GameObject("W07 targeted evidence").AddComponent<WeatherW07Probe>();p.mode=key;p.output=a[i+1];Directory.CreateDirectory(p.output);break;}}
 ...
 IEnumerator Pictures(){
  ...
  env.FlashOverride=0;variant.ShowBaseline(false);yield return new WaitForSecondsRealtime(.5f);yield return Snap("abbey-W07-normal");
  env.FlashOverride=1;env.StrongOverride=false;yield return null;yield return Snap("abbey-W07-flash");env.StrongOverride=true;yield return null;yield return Snap("abbey-W07-strong");env.FlashOverride=0;yield return new WaitForSecondsRealtime(1);yield return Snap("abbey-after-flash");
  foreach(float angle in new[]{.59f,.05f,1.1f}){CloseView(angle);variant.ShowBaseline(true);yield return new WaitForSecondsRealtime(.4f);yield return Snap("canopy-W06-"+angle);variant.ShowBaseline(false);yield return new WaitForSecondsRealtime(.4f);yield return Snap("canopy-W07-"+angle);}
 ...
 IEnumerator Start(){yield return new WaitForSecondsRealtime(2);env=FindAnyObjectByType<WorldEnvironment>();motor=env.player;control=FindAnyObjectByType<WorldCamera>();run=motor.GetComponent<WorldRunInput>();camera=control.GetComponent<Camera>();variant=FindAnyObjectByType<WeatherW07Variant>();control.nativeInput=false;run.keyboardEnabled=false;
  if(mode=="-w07Capture")yield return Pictures();else if(mode=="-w07Traverse")yield return Traverse();else yield return Performance();Save();yield return new WaitForSecondsRealtime(.5f);Application.Quit(failures==0&&errors.Count==0?0:1);
 }
}
}
