...
public sealed partial class WorldProbe:MonoBehaviour {
 ...
 [Serializable]public class Report{public string mode,device,note;public int width,height,failures,samples,clicks,blocked,drags,zooms,resets,safetyStops,fixturePlacements,nativeShiftFrames,shiftChanges;public float distance,nativeRunDistance;public Vector3 position;public string[] checks,errors;public Performance[] performance;}
 ...
 int failures,samples,fixturePlacements,nativeShiftFrames;float distance,nativeRunDistance;StreamWriter trace;float nextManualSave;bool fatal;
 [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]static void Install(){
  var args=Environment.GetCommandLineArgs();foreach(string option in new[]{"-worldQA","-worldEdges","-worldManual","-worldPerformance","-worldCapture","-worldWeatherQA","-worldFilm","-worldMaterials","-worldAblation"}){int i=Array.IndexOf(args,option);if(i<0||i+1>=args.Length)continue;var p=new GameObject("Opt-in world evidence").AddComponent<WorldProbe>();p.output=args[i+1];p.mode=option;Directory.CreateDirectory(p.output);Application.runInBackground=true;break;}
 }
 ...
 void Save(){if(!motor||!control)return;File.WriteAllText(output+"/report.json",JsonUtility.ToJson(new Report{mode=mode,device=SystemInfo.graphicsDeviceName,note="Opt-in runtime evidence. Synthetic pointer checks and automatic placement are not native device acceptance. Natural frame cadence is not display-present timing.",width=Screen.width,height=Screen.height,failures=failures,samples=samples,distance=distance,clicks=control.clicks,blocked=control.blocked,drags=control.drags,zooms=control.zooms,resets=control.resets,safetyStops=motor.SafetyStops,position=motor.transform.position,fixturePlacements=fixturePlacements,nativeShiftFrames=nativeShiftFrames,nativeRunDistance=nativeRunDistance,shiftChanges=run?run.shiftChanges:0,checks=checks.ToArray(),errors=errors.ToArray(),performance=performances.ToArray()},true));}
 void Update(){
  if(!motor)return;
  if(mode=="-worldManual"){
   distance+=motor.Distance;samples++;
   if(Input.GetKey(KeyCode.LeftShift)||Input.GetKey(KeyCode.RightShift)){nativeShiftFrames++;nativeRunDistance+=motor.Distance;}
   if(Input.GetKeyDown(KeyCode.F6))ManualFixture(45);if(Input.GetKeyDown(KeyCode.F7))ManualFixture(180);if(Input.GetKeyDown(KeyCode.F8))ManualFixture(342);
 ...
 void ManualFixture(float d){motor.Stop();motor.transform.position=WorldLayout.Point(d);control.SetZoom(11.1f);fixturePlacements++;checks.Add("MANUAL FIXTURE placement at "+d+"; not continuous traversal or a native movement click");Save();}
 void OnGUI(){if(mode=="-worldManual")GUI.Label(new Rect(28,Screen.height-58,1100,22),"QA fixtures: F6 river / F7 abbey / F8 snow    |    F9 capture    |    F10 save and exit");}
 ...
 IEnumerator Start(){
  ...
  else if(mode=="-worldMaterials")yield return MaterialDiagnostics();
  else if(mode=="-worldAblation")yield return Ablation();
