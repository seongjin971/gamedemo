...
public sealed partial class WorldProbe:MonoBehaviour {
 ...
 [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.AfterSceneLoad)]static void Install(){
  var args=Environment.GetCommandLineArgs();foreach(string option in new[]{"-worldQA","-worldEdges","-worldManual","-worldPerformance","-worldCapture","-worldWeatherQA","-worldFilm","-worldMaterials"}){int i=Array.IndexOf(args,option);if(i<0||i+1>=args.Length)continue;var p=new GameObject("Opt-in world evidence").AddComponent<WorldProbe>();p.output=args[i+1];p.mode=option;Directory.CreateDirectory(p.output);Application.runInBackground=true;break;}
 }
