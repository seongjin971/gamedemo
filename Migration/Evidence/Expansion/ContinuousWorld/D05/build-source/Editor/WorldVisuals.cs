...
public static partial class WorldVisuals {
 ...
 static Material Slot(string name){
  ...
  if(name.Contains("ChapelPaving")){var wet=Mat("Wet_"+name,new Color(.65f,.69f,.71f),.8f);wet.SetTexture("_BaseMap",stoneTexture);wet.shader=Shader.Find("Vesper/WorldWetStone");wet.SetFloat("_Wetness",.8f);return wet;}
 ...
 public static void Waterside(GameObject collision,WorldEnvironment world,string generated){
  ...
  foreach(var renderer in collision.GetComponentsInChildren<MeshRenderer>())if(renderer.name.StartsWith("Continuous terrain")){
   ...
   for(int i=0;i<v.Length;i++){float d=-v[i].z;var w=WorldLayout.Weights(d);float dist=Mathf.Abs(v[i].x-WorldLayout.Center(d));if(d>34&&d<67)dist=Mathf.Min(dist,Mathf.Abs(v[i].x-WorldLayout.Center(48.5f)));
    float path=1-Mathf.SmoothStep(0,1,Mathf.InverseLerp(2.5f,5,dist));float slope=1-normals[i].y;
  ...
  var center=WorldLayout.Center(48.5f);var bridgeVisual=Model(detail.transform,"BridgeHeroV2/CW_StoneBridgeHeroV2_14m",new Vector3(center,WorldLayout.Level(48.5f),-48.5f),Vector3.one,0);ReshapeStone(bridgeVisual,true);
  ...
  WorldBuild.MeshObject(detail.transform,"One reflective river plane",new List<Vector3>{new Vector3(-90,-.95f,-34),new Vector3(90,-.95f,-34),new Vector3(90,-.95f,-61),new Vector3(-90,-.95f,-61)},new List<int>{0,1,2,0,2,3},surface,31,false);
  ...
  foreach(var q in new[]{new Vector3(6.5f,56.5f,.55f),new Vector3(-8.5f,57.5f,.9f),new Vector3(-6.3f,37.5f,.85f),new Vector3(9,38,.5f)})Model(detail.transform,"CW_HF_Limestone",new Vector3(center+q.x,-.65f,-q.y),new Vector3(q.x==-8.5f?1.8f:q.x>0?.95f:1.35f,q.z,q.x>0?.85f:1.2f),R(0,360),true,true);
  Model(detail.transform,"CW_HF_Limestone",new Vector3(center-5.5f,-.65f,-58.7f),new Vector3(.65f,.52f,.75f),205,true,true);
