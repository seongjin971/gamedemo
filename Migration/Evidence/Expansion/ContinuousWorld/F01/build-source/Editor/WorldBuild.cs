...
public static class WorldBuild {
 ...
 public static string ScenePath=>"Assets/Vesper/Scenes/Expansion/VesperContinuousWorld"+(revision!=null&&revision.StartsWith("F")?"_"+revision:"")+".unity";
 ...
 static void Terrain(Transform parent){
  for(int z0=-448;z0<32;z0+=32)for(int x0=-80;x0<80;x0+=32){
   int cells=z0<=-352?64:z0>=-96&&x0>=-48&&x0<=16?64:z0<=-224&&x0>=-48&&x0<=16?32:16;float spacing=32f/cells;
   ...
   var chunk=MeshObject(parent,"Continuous terrain "+x0+" "+z0,verts,tris,earth,28,true,colors.ToArray());
 ...
 public static void Prepare(){try{
  ...
  var world=Lighting(player,camera);string stage=Arg("-worldStage","A");if(stage!="A")WorldVisuals.Waterside(terrain,world,generated);if(stage=="C"||stage=="D"||stage=="E")WorldVisuals.Ruins(terrain,world);if(stage=="D"||stage=="E")WorldVisuals.SnowPass(terrain,world);if(revision.StartsWith("F"))WorldVisuals.RecomposeVisualF(terrain,world);Bake(terrain);Physics.SyncTransforms();
